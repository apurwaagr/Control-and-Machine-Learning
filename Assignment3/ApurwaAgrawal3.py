#Assignment3 Apurwa Agrawal Matrikel-Nr. 23161645
...
#y = np.concatenate((np.ones((1, 6)), np.zeros((1, 5))), axis=1)
#y = np.concatenate((y, np.concatenate((np.zeros((1, 6)), np.ones((1, 5))), axis=1)), axis=0)
#y = np.array[[np.ones((1,5)),np.zeros((1,5))], [np.zeros((1,5)),np.ones((1,5))]]
...
def train_neural_network(x1, x2, y, activation, learning_rate):
    ...
    plt.savefig(f'{activation.__name__}_loss_lr_{learning_rate}.png')


    # Visualization of output
    visualize_output(theta1, theta2, activation, x1, x2, learning_rate)


def visualize_output(theta1, theta2, activation, x1, x2, learning_rate):
    ...
    plt.figure()
    plt.contourf(xx, yy, a3, levels=[-1, 0.5, 1], colors=['white', 'grey'], alpha=0.2)
    plt.contour(xx, yy, a3, levels=[0.5], colors='black')  # Add boundary line
    plt.scatter(x1[:6], x2[:6], c='r', marker='o', label='Category A')
    plt.scatter(x1[6:], x2[6:], c='b', marker='x', label='Category B')
    plt.xlabel('x1')
    plt.ylabel('x2')
    plt.title(f'{activation.__name__} Activation Function Decision Boundaries (Learning Rate: {learning_rate})')
    plt.legend()
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.xticks([0, 1])
    plt.yticks([0, 1])
    plt.grid(True)
    plt.savefig(f'{activation.__name__}_decision_boundaries_lr_{learning_rate}.png')
    plt.show()
